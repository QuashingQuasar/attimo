...
async function getSanityPost(slug) {
  ...
  const res = await fetch(
    `https://${SANITY_PROJECT_ID}.api.sanity.io/v2021-10-21/data/query/${SANITY_DATASET}?query=${query}`
  );
...
export default {
  async fetch(request, env) {
    ...
    if (blogMatch) {
      try {
        const post = await getSanityPost(blogMatch[1]);
        if (post) {
          const title = post.seoTitle || post.title || 'ATTIMO Specialty Extra Virgin Olive Oil';
          const description = post.seoDescription || post.excerpt || '';
          const image = post.coverImageUrl
            ? `${post.coverImageUrl}?w=1200&auto=format`
            : 'https://attimo-oil.com/og-image.jpg';
    ...
    // Everything else — serve static assets from Pages deployment
